...
sdf_select <- function(x, ..., .aliases, .drop_parents=TRUE, .full_name=FALSE) {
  ...
  # need to pull out nested field refs since eval_select will not find them
  arg_strings <- vapply(dots, rlang::as_name, character(1))
  arg_strings <- stats::setNames(arg_strings, names(dots))
  id <- is_nested_field_ref(arg_strings)

  # collect field names to select as strings
  # `!!!` will splice dots[!id] into `...` of eval_select
  proxy <- stats::setNames(seq_along(colnames(x)), colnames(x))
  pos <- tidyselect::eval_select(rlang::expr(c(!!!dots[!id])), data = proxy)
  top_level_vars <- stats::setNames(colnames(x)[pos], names(pos))
  ...
  if (.drop_parents) {
    nested_fields_accessed <- select_cols[grepl(".", select_cols, fixed = TRUE)] |>
      ...
      vapply(function(y) y[[1]], character(1))
  ...
  if (missing(.aliases)) {
    ...
    if (.full_name) {
      ...
      nested_names <- nested_vars |>
        strsplit(split=".", fixed=TRUE) |>
        vapply(function(y) y[[length(y)]], character(1))
  ...
  columns <- mapply(FUN=function(arg, name) {
    sparklyr::invoke(sdf, "col", arg) |>
      sparklyr::invoke("alias", name)
  }, select_cols, .aliases)

  # do select
  outdf <- sparklyr::invoke(sdf, "select", columns)
