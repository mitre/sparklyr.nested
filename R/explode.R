...
sdf_explode <- function(x, column, is_map=FALSE, keep_all=FALSE) {
  ...
  columns <- lapply(cols, function(field) {
    sdf_col <- sparklyr::invoke(sdf, "col", field)
    if (field == col_name) {
      sdf_col <- sparklyr::invoke_static(sc, method=scala_method,
                               class="org.apache.spark.sql.functions",
                               sdf_col)
      if (!is_map)
        sdf_col <- sparklyr::invoke(sdf_col, "alias", field)
    }
  ...
  # do select
  outdf <- sdf |>
    sparklyr::invoke("select", columns)
